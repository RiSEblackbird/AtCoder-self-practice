N = int(input())
nums = list([i+1,True] for i in range(N))
# nums.append([i+1,True] for i in range(100))
print(nums)
...
while p <= N:
  for i in range(p+1,N):
    if nums[i][1] == False:
      ...
      print("p={}, i={}, nums[i][0]={}".format(p,i,nums[i][0]))
      nums[i][1] = False
  p += 1
print(nums)

ans = set(list(nums[j][0] if nums[j][1] == True else nums[j].clear() for j in range(N)))
ans.remove(None)
print(*list(ans))
